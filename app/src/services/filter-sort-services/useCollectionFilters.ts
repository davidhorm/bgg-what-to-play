...
export type FilterControl<T> = {
  getInitialState: (searchParams: URLSearchParams) => T;
...
export type CollectionFilterState = {
  ...
  username: ReturnType<typeof usernameService.getInitialState>;

  /** If `true`, then show the invalid Player Count outside of the game's actual min/max Player Count. */
  showInvalidPlayerCount: ReturnType<
    typeof showInvalidPlayerCountService.getInitialState
  >;
   ...
   */
  playerCountRange: ReturnType<typeof playerCountRangeService.getInitialState>;
   ...
   */
  playtimeRange: ReturnType<typeof playtimeService.getInitialState>;

  /**
   * The Complexity `[minRange, maxRange]` the user wants to filter the collection.
   * - Valid `minRange` and `maxRange` values are 1-5.
   */
  complexityRange: ReturnType<typeof complexityService.getInitialState>;

  /** If `true`, then show expansions in collection. */
  showExpansions: ReturnType<typeof showExpansionsService.getInitialState>;
   ...
   */
  showRatings: ReturnType<typeof showRatingsService.getInitialState>;
   ...
   */
  ratingsRange: ReturnType<typeof ratingsService.getInitialState>;

  /** If `true`, then show games where all of the filtered player counts are not recommended. */
  showNotRecommended: ReturnType<
    typeof showNotRecommendedService.getInitialState
  >;

  /** If `true`, then `console.log` messages to help troubleshoot. */
  isDebug: ReturnType<typeof isDebugService.getInitialState>;
};
...
export const buildSearchParamsFromState = (
  ...
  const searchParams = base ? new URLSearchParams(base) : new URLSearchParams();

  // each service sets its own params using the passed-in URLSearchParams
  usernameService.setQueryParam(searchParams, state);
  playerCountRangeService.setQueryParam(searchParams, state);
  playtimeService.setQueryParam(searchParams, state);
  complexityService.setQueryParam(searchParams, state);
  ratingsService.setQueryParam(searchParams, state);
  showRatingsService.setQueryParam(searchParams, state);
  showExpansionsService.setQueryParam(searchParams, state);
  showNotRecommendedService.setQueryParam(searchParams, state);
  showInvalidPlayerCountService.setQueryParam(searchParams, state);
  isDebugService.setQueryParam(searchParams, state);
...
const createReducer =
  ...
  (state, action): CollectionFilterState => {
    const newState = (
...
export const applyFiltersAndSorts =
  ...
  (games: SimpleBoardGame[]) => {
    if (filterState.isDebug) printDebugMessage("All Games", games);
...
export const useCollectionFilters = (props?: Props) => {
  ...
  const initialFilterState = {
    username: usernameService.getInitialState(searchParams),
    showInvalidPlayerCount:
      showInvalidPlayerCountService.getInitialState(searchParams),
    playerCountRange: playerCountRangeService.getInitialState(searchParams),
    playtimeRange: playtimeService.getInitialState(searchParams),
    complexityRange: complexityService.getInitialState(searchParams),
    showExpansions: showExpansionsService.getInitialState(searchParams),
    showRatings: showRatingsService.getInitialState(searchParams),
    ratingsRange: ratingsService.getInitialState(searchParams),
    showNotRecommended: showNotRecommendedService.getInitialState(searchParams),
    isDebug: isDebugService.getInitialState(searchParams),
  };
  ...
  useEffect(() => {
    if (!filterState.username) return;

    const currentSearchParams = searchParams.toString();
    const qs = buildSearchParamsFromState(filterState, searchParams);
    const newSearchParams = qs.toString();

    if (newSearchParams === currentSearchParams) return;

    const newUrl = `${pathname}${newSearchParams ? `?${newSearchParams}` : ""}`;
    router.replace(newUrl, { scroll: false });
  }, [filterState, router, pathname, searchParams]);

  const initialSortState = props?.isInitialSortStateEmpty
    ? []
    : getInitialSortState(searchParams, defaultSortConfigs);
  const [selectedSorts, setSelectedSorts] = useState(initialSortState);

  const onSelectedSortsChange = useCallback(
    (state: SortConfig[], sortParamKey: string) => {
      const qs = new URLSearchParams(searchParams);
      if (!qs.has("username")) return;

      const currentSearchParams = qs.toString();
      ...
      qs.set(sortParamKey, queryParamValue);
      const newSearchParams = qs.toString();
  ...
  const initialSliderValues: Record<string, [number, number]> =
    sliderSetActions.reduce(
      (prevVal, currVal, index) => ({
        ...prevVal,
        [index]: currVal.sliderControl.getInitialState(searchParams),
      }),
      {},
    );
  ...
  return {
    ...
    toggleSelectedSort: toggleSelectedSort(
      setSelectedSorts,
      onSelectedSortsChange,
      defaultSortConfigs,
    ),
    deleteSelectedSort: deleteSelectedSort(
      setSelectedSorts,
      onSelectedSortsChange,
    ),
  };
